...
export default async function CalendarPage() {
  ...
  return <DetailPage eyebrow="OFFICIAL ECONOMIC CALENDAR" title="주요 경제 일정" description="BLS, BEA, 미 연방준비제도와 한국은행의 공식 발표 일정을 한국시간으로 통합합니다.">
    ...
    <Section title="향후 24시간"><div className="calendar-list">{schedules.map(item => {
      ...
      return <a href={item.source_url} target="_blank" rel="noreferrer" key={item.id}>
        ...
        <div><small>{item.category} · {sourceLabels[item.source]}</small><b>{item.title}</b></div>
        <em className={`badge ${item.importance}`}>{importanceLabels[item.importance]}</em><i>원문 ↗</i>
      </a>;
    })}</div></Section>
