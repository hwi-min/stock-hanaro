...
export function Dashboard({ data }: { data: DashboardType }) {
  return <main className="dashboard">
    ...
    <div className="grid main-grid">
      ...
      <Section title="이번 주 주요 일정" href="/calendar"><div className="list home-scroll">{data.schedules.map(item => <div key={item.id}><time>{formatScheduleTime(item.scheduled_at)}</time><b>{item.title}</b><span className={`badge ${item.importance}`}>{item.importance === "high" ? "중요" : item.importance === "medium" ? "보통" : "낮음"}</span></div>)}</div></Section>
      <Section title="오늘의 주요 이슈" href="/issues"><div className="issue-list home-scroll">{data.issues.map(item => <Link key={item.id} href={`/issues/${item.id}`}><div><b>{item.title}</b><p>{item.summary}</p></div><span className={`sentiment ${item.sentiment}`}>{sentimentLabel[item.sentiment]}</span></Link>)}</div></Section>
    </div>

    <div className="grid lower-grid">
      <Section title="국제금융센터 요약" href="/briefings"><div className="cards home-scroll">{data.kcif.map(item => <article key={item.id}><span className="topic">{item.topic}</span><h3>{item.title}</h3><p>{item.summary}</p><small>KCIF 원문 기반 요약</small></article>)}</div></Section>
      <Section title="중요 공시" href="/disclosures"><div className="table home-scroll">{data.disclosures.map(item => <a className="disclosure-row" href={disclosureUrl(item)} target="_blank" rel="noopener noreferrer" key={item.id} aria-label={`${item.company} ${item.title} DART 원문 열기`}><b>{item.company}</b><span>{item.title}</span><em className={`badge ${item.importance}`}>{item.importance === "high" ? "중요" : "보통"}</em></a>)}</div></Section>
    </div>
    ...
    <Section title="최신 증권사 리서치" href="/research" className="home-research-panel"><div className="home-research-list home-scroll">{data.research.map(item => <article key={item.id}>
