...
def search(q: str = Query(min_length=1, max_length=50), db: Session = Depends(get_db)):
    term = q.strip().lower()
    stocks = [{"type": "stock", "id": symbol, "symbol": symbol, "name": item["name"],
               "market": item["market"], "label": f"{item['name']} · {symbol}"}
              for symbol, item in stock_catalog().items()
              if term in symbol.lower() or term in item["name"].lower()]
...
async def stock_detail(symbol: str, interval: str = Query(default="daily", pattern="^(daily|minute)$"),
    ...
    if metadata["market"] == "kr":
        ...
        chart = await (kis_client.domestic_minute_chart(symbol) if interval == "minute"
                       else kis_client.domestic_daily_chart(symbol))
        currency = "KRW"
    else:
